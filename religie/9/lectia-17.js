...
const titleText = "Învierea Domnului"; // Textul titlului
...
// Inițializare video
// Funcția handleVideos actualizată pentru a permite redarea de la un timestamp specific
function handleVideos() {
  ...
  videoPlaceholders.forEach((placeholder) => {
    ...
    if (videoType === "youtube") {
      ...
      // Verifică dacă avem un timestamp în data-src
      let youtubeId = videoSrc;
      let startTime = 0;

      // Verifică dacă există & sau ? în src, care indică parametri
      if (videoSrc.includes("&t=") || videoSrc.includes("?t=")) {
        // Format: videoId&t=123s sau videoId?t=123s
        const parts = videoSrc.split(/[?&]t=/);
        youtubeId = parts[0];
      ...
      // Adaugă startTime ca parametru în URL
      iframe.src = `https://www.youtube.com/embed/${youtubeId}?start=${startTime}`;
      iframe.allow =
      ...
      // Cod pentru video local - neschimbat
      const container = document.createElement("div");
