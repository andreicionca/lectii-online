...
const titleText = '„Țara Sfântă”'; // Textul titlului
...
const studentTracking = JSON.parse(localStorage.getItem('studentTracking')) || {};
const classTracking = studentTracking[currentClass] || {};
const currentStudents = JSON.parse(localStorage.getItem('selectedStudents')) || [];
...
function toggleFullscreen() {
  if (!document.fullscreenElement) {
    document.documentElement.requestFullscreen().catch((err) => {
      showError('Eroare la intrarea în ecran complet. Verificați setările browser-ului.');
    });
...
function selectRandomStudent() {
  ...
  if (availableStudents.length === 0) {
    const minSelections = Math.min(...currentStudents.map((s) => classTracking[s]));
    availableStudents = currentStudents.filter((s) => classTracking[s] === minSelections);
  }
...
function togglePlayPause() {
  if (playPauseBtn.querySelector('[data-lucide]').dataset.lucide === 'refresh-ccw') {
    restartTeleprompter();
    return;
  }
...
document.addEventListener('fullscreenchange', () => {
  fullscreenToggle.setAttribute(
    'data-tooltip',
    document.fullscreenElement ? 'Ieșire din ecran complet' : 'Comută ecran complet'
  );
});
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (typeof teleprompterView !== 'undefined' && !teleprompterView.classList.contains('hidden')) {
    handleVideos();
  }
